...
The `/index` pipeline stores the canonical search-facing fields automatically, including `filename`, `ai_model`, `ai_rundate`, and `photo_date` from the request `photo_date` field or uploaded image EXIF fallback.
