...
Indexing requests must upload photos in `multipart/form-data` using repeated `image` file fields and matching `uuid`, `filename`, and `photo_date` fields. The capture date contract is `photo_date`: clients send the photo capture date in a `photo_date` field, and the backend stores that value as canonical metadata `photo_date`. Do not send legacy date fields such as `date_time` or `photos_date`. Additional form fields are preserved as metadata, so EXIF or other image metadata can be sent alongside the upload. Server-side file paths in the request body are no longer supported.
