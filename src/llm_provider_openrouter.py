"""
OpenRouter Provider for metadata generation using the OpenRouter Chat Completions API
"""
import json
...
from config import logger, OPENROUTER_BASE_URL


class OpenRouterProvider(LLMProviderBase):
    """
    Provider for OpenRouter's unified API.
    Supports any vision-capable chat completion model OpenRouter proxies.
    """

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.api_key = config.get("api_key")
        self.base_url = config.get("base_url", OPENROUTER_BASE_URL).rstrip("/")
    ...
    def generate_metadata(self, request: MetadataGenerationRequest) -> MetadataGenerationResponse:
        """
        Generate metadata using the OpenRouter API.
        ...
        try:
            ...
            response_format = self._prepare_openrouter_response_format(request)
            ...
            self._log_llm_payload("OpenRouter metadata request", payload)
            response_data = self._post_chat_completion(api_key, payload)

            choice = self._first_choice(response_data)
            finish_reason = choice.get("finish_reason")
            if finish_reason and finish_reason != "stop":
                error_msg = f"OpenRouter generation failed: {finish_reason}"
            ...
            logger.debug(f"OpenRouter raw response: {content}")
            ...
            logger.error(f"Error generating metadata with OpenRouter: {e}", exc_info=True)
    ...
    def generate_quality_scores(self, request: QualityScoreRequest) -> QualityScoreResponse:
        """
        Generate quality scores using the OpenRouter API.
        ...
        try:
            ...
            self._log_llm_payload("OpenRouter quality scoring request", payload)
            response_data = self._post_chat_completion(api_key, payload)

            choice = self._first_choice(response_data)
            finish_reason = choice.get("finish_reason")
            if finish_reason and finish_reason != "stop":
                error_msg = f"OpenRouter quality generation failed: {finish_reason}"
                logger.error(error_msg)
                usage = response_data.get("usage") or {}
                return QualityScoreResponse(
                    uuid=request.uuid,
            ...
            content = self._extract_message_content(choice)
            logger.debug(f"OpenRouter quality response: {content}")
            parsed_data = json.loads(self._clean_json_response(content))
            ...
            logger.error(f"Failed to parse JSON from OpenRouter quality response: {e}")
            return QualityScoreResponse(
                uuid=request.uuid,
                success=False,
                error=f"JSON parsing error: {str(e)}",
            ...
            logger.error(f"Error generating quality scores with OpenRouter: {e}", exc_info=True)
            return QualityScoreResponse(uuid=request.uuid, success=False, error=str(e))

    def list_available_models(self) -> list:
        """
        List vision-capable models from OpenRouter's model catalog.

        Returns:
            List of OpenRouter model identifiers (e.g. "anthropic/claude-3.7-sonnet"),
            usable verbatim as the `model` field in later requests.
        """
        api_key = self._get_api_key(None)
        if not api_key:
            logger.info("OpenRouter API key not configured; returning no OpenRouter models")
            return []

        try:
            models_url = f"{self.base_url}/models"
            self._log_llm_payload(
                "OpenRouter list models request",
                {
                    "method": "GET",
                    "url": models_url,
                    "headers": {
            ...
            dynamic_models = []
            for model in response_data.get("data") or []:
            ...
            if dynamic_models:
                logger.info(f"Returning {len(dynamic_models)} OpenRouter models from API")
                return dynamic_models

            logger.warning("OpenRouter API returned no matching vision chat models")
        except Exception as e:
            logger.error(f"Error listing OpenRouter models from API: {e}", exc_info=True)
    ...
    def _is_vision_chat_model(self, model_id: Optional[str], model: Dict[str, Any]) -> bool:
        ...
        # Fallback for older catalog entries using the single "modality" string.
        modality = architecture.get("modality") or ""
        return "image" in modality.split("->")[0]

    def _prepare_openrouter_response_format(
    ...
    def _format_http_error(self, response: requests.Response) -> str:
        ...
        return f"OpenRouter API request failed with status {response.status_code}: {message}"
